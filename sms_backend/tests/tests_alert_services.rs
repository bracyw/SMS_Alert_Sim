...
async fn test_create_sim_senders_num() {
    let senders = create_sim_senders(100.0, 100.0, 1.0, 1.0);
    assert_eq!(100, senders.unwrap().len());
}

#[tokio::test]
async fn test_create_sim_senders_failure_rate() {
    let senders = create_sim_senders(100.0, 0.0, 1.0, 1.0);
...
#[tokio::test]
async fn test_create_sim_senders_wait_time() {
    let senders = create_sim_senders(100.0, 0.0, 5.0, 0.0);

    let mut rand = rand::thread_rng();
    let rand_index = rand.gen_range(0..100);
    let start_time= Instant::now();
    senders.unwrap().get(rand_index).unwrap().send("hello there".to_string()).await;
    assert_eq!(5, start_time.elapsed().as_secs(), "sender service should have created senders that send in 5 seconds");
}

#[tokio::test]
async fn test_send_msgs_expected_alert() {
    let pm = Arc::new(ProgressMonitor::new());
    let sender_service = create_sender_service(10, 0, 1.0, 0.0, pm).await.unwrap();
    ...
    assert_eq!(result.total_time_to_send, 1);
}
