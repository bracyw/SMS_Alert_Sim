...
use sms_backend::{models::alert_models::Alert, structures::{producer::SmsProducer, progress_monitor::ProgressMonitor}, utils::sms_utils::sender_utils::create_sender_service};
...
async fn test_producer_basic_send() {
    ...
    let sender_service = create_sender_service(1, 100, 1.0, pm).await.unwrap();
...
//     let pm = Arc::new(ProgressMonitor::new());
//     let sender_service = create_sender_service(90, 100, 0.5, pm).await.unwrap();
...
//     let mut task_handle = FuturesUnordered::new();
  
//     task_handle.push(tokio::spawn(async move {
//         producer1.send_msgs(None, 30).await
//     }));
//     task_handle.push(tokio::spawn(async move {
//         producer2.send_msgs(None, 30).await
//     }));
//     task_handle.push(tokio::spawn(async move {
//         producer3.send_msgs(None, 30).await
//     }));

//     while let Some(result) = task_handle.next().await {
...
async fn multi_producer_same_service() {
    let pm = Arc::new(ProgressMonitor::new());
    let sender_service = create_sender_service(100, 100, 1.0, pm).await.unwrap();
